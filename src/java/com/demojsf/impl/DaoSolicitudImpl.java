...
public class DaoSolicitudImpl implements DaoSolicitud<Solicitud> {
    ...
    public void update(Solicitud s) {
        ...
        try {
            ...
            PreparedStatement pst = connect.prepareStatement("Update Solicitud set fecha_solicitud=?,id_area=?,nombre_usuario=?,cargo_solicitud=?,asunto_solicitud=?,observacion=?,id_estado=?,id_tipo_solicitud=?,id_lista_solicitud=? where id_solicitud=?");
            pst.setInt(10, s.getId_solicitud());
